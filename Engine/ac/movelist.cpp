...
void MoveList::SetDirectMove(bool move_direct)
{
...
float MoveList::CalcPartsFromProgress(int stage, float progress) const
{
    ...
    const float xparts = (permove[stage].X != 0.f) ? (std::abs(target.X - pos[stage].X) * progress) / permove[stage].X : 0.f;
    const float yparts = (permove[stage].Y != 0.f) ? (std::abs(target.Y - pos[stage].Y) * progress) / permove[stage].Y : 0.f;
