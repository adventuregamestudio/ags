...
static void OnTouchPointer(int pointer_id, TouchPhase phase, const Point &position)
{
    ...
    // Detach and invalidate script object when the pointer is released,
    // this effectively makes this object work as a single pointer gesture.
    if (phase == TouchPhase::Up || phase == TouchPhase::Undefined)
    {
        DetachScriptTouchPointer(g_TouchPointers[pointer_id]);
    }
