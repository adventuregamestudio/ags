...
void quit_tell_editor_debugger(const CString &qmsg)
{
    if (editor_debugging_initialized)
    {
        if ((qmsg[0] == '!') && (qmsg[1] != '|'))
        {
            handledErrorInEditor = send_exception_to_editor(qmsg.Mid(1));
        }
...
void quit_check_dynamic_sprites(const CString &qmsg)
{
...
void quit_shutdown_platform(const CString &qmsg)
{
...
void quit_check_for_error_state(CString &qmsg, CString &alertis)
{
    int substring_left = 0;
    if (qmsg[substring_left]=='|') ; //qmsg++;
    else if (qmsg[substring_left]=='!') { 
        substring_left++;

        if (qmsg[substring_left] == '|')
            ...
            alertis = "An error has occurred. Please contact the game author for support, as this "
        ...
        alertis = "An internal error has occurred. Please note down the following information.\n"
...
void quit_message_on_exit(const CString &qmsg, CString &alertis)
{
...
void quit_print_last_fps(const CString &qmsg)
{
