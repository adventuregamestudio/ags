...
HError create_game_scripts()
{
    ...
    {
        if (!scriptModules[i])
            return new Error("Script module (%zu) '%s' was not loaded", i, scriptModuleNames[i].GetCStr());
        moduleInst[i] = ccInstance::CreateFromScript(scriptModules[i]);
        if (!moduleInst[i])
            return new Error("Failed to create the script module (%zu) '%s':\n%s",
    ...
    for (size_t i = 0; i < numScriptModules; ++i)
    {
        ...
        if (!fork)
            return new Error("Failed to create non-blocking fork for the script module (%zu) '%s':\n%s",
                i, scriptModules[i]->GetScriptName().c_str(), cc_get_error().ErrorString.GetCStr());
