...
script object files (*.o) from the project build directory (default: <output>/_temp).
These are normally produced by the AGS Editor build; agdta does not compile scripts.
...
int main(int argc, char *argv[])
{
    ...
    scripts_dir = AGS::Common::ResolveToAbsolutePath(scripts_dir);
    ...
    dta_path = AGS::Common::ResolveToAbsolutePath(dta_path);

    const size_t slash = dta_path.rfind('/');
    if (slash != std::string::npos)
        AGS::Common::EnsureDirectory(dta_path.substr(0, slash));
