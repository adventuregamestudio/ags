    ...
    class GuiComponent : BaseComponentWithFolders<GUI, GUIFolder>
    {
        ...
        private void ScanAndReportMissingEventHandlers(GenericMessagesArgs args)
        {
            ...
            foreach (GUI gui in _agsEditor.CurrentGame.GUIs)
            {
                ...
                _agsEditor.Tasks.ScanAndReportMissingEventHandlers("GUI", ngui.Name, ngui.ID, null, ngui.ScriptModule, events.ToArray(), true, errors);
                ...
                {
                    events = new List<Tuple<string, string>>();
                    events.Add(new Tuple<string, string>("OnClick", button.OnClick));
                    _agsEditor.Tasks.ScanAndReportMissingEventHandlers(button.ControlType, button.Name, button.ID, objectContext, ngui.ScriptModule, events.ToArray(), true, errors);
                }

                foreach (var listbox in listboxes)
                {
                    events = new List<Tuple<string, string>>();
                    events.Add(new Tuple<string, string>("OnSelectionChanged", listbox.OnSelectionChanged));
                    _agsEditor.Tasks.ScanAndReportMissingEventHandlers(listbox.ControlType, listbox.Name, listbox.ID, objectContext, ngui.ScriptModule, events.ToArray(), true, errors);
                }

                foreach (var slider in sliders)
                {
                    events = new List<Tuple<string, string>>();
                    events.Add(new Tuple<string, string>("OnChange", slider.OnChange));
                    _agsEditor.Tasks.ScanAndReportMissingEventHandlers(slider.ControlType, slider.Name, slider.ID, objectContext, ngui.ScriptModule, events.ToArray(), true, errors);
                }

                foreach (var textbox in textboxes)
                {
                    events = new List<Tuple<string, string>>();
                    events.Add(new Tuple<string, string>("OnActivate", textbox.OnActivate));
                    _agsEditor.Tasks.ScanAndReportMissingEventHandlers(textbox.ControlType, textbox.Name, textbox.ID, objectContext, ngui.ScriptModule, events.ToArray(), true, errors);
                }
            }
        }
    }
