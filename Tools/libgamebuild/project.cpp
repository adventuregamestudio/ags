    ...
    if (sprite_loader_->Open(game_dir))
    {
        ...
        if (game_data_)
        {
            ...
            else
            {
                ...
                for (size_t i = 0; i < game_data_->sprites.size(); ++i)
                    id_to_idx[game_data_->sprites[i].id] = i;

                for (const auto& m : all)
                {
                    if (!m.exists)
                        continue;
                    auto it = id_to_idx.find(m.id);
                    if (it != id_to_idx.end())
                    {
                        auto& spr = game_data_->sprites[it->second];
                        spr.width = m.width;
                        spr.height = m.height;
                    ...
                    else
                    {
                        ...
                        s.height = m.height;
                        s.color_depth = m.color_depth * 8;
                        auto it_src = game_data_->pending_sprite_source_.find(s.id);
                        if (it_src != game_data_->pending_sprite_source_.end()) {
                            s.source = std::move(it_src->second);
                            game_data_->pending_sprite_source_.erase(it_src);
                        }
                        auto it_res = game_data_->pending_sprite_resolution_.find(s.id);
                        if (it_res != game_data_->pending_sprite_resolution_.end()) {
                            s.resolution = it_res->second;
...
void Project::ReloadSpriteCache()
{
    if (!loaded_ || project_dir_.empty())
        return;
...
bool Project::LoadFromAGF(const std::string& agf_path)
{
    ...
    const std::string game_dir = project_dir_.empty()
        ? ResolveToAbsolutePath(fs::path(agf_path).parent_path().string())
        : project_dir_;
