...
extern "C"
{
  ...
  int read_new_new_enc_format_clib(MultiFileLibNew * mfl, CDataStream *ci_s, int libver)
  {
    ...
    fread_data_enc(&mfl->offset[0], sizeof(long), mfl->num_files, ci_s);
    fread_data_enc(&mfl->length[0], sizeof(long), mfl->num_files, ci_s);
  ...
  int read_new_new_format_clib(MultiFileLibNew * mfl, CDataStream *ci_s, int libver)
  {
    ...
    ci_s->ReadArray(&mfl->offset[0], sizeof(long), mfl->num_files);
    ci_s->ReadArray(&mfl->length[0], sizeof(long), mfl->num_files);
