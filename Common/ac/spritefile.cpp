...
HError SpriteFile::LoadSprite(sprkey_t index, PixelBuffer &sprite)
{
    ...
    if (index < 0 || (size_t)index >= _spriteData.size())
        return new Error(String::FromFormat("LoadSprite: slot index %d out of bounds (%d - %zu).",
    ...
    if (!err)
    {
        return new Error(String::FromFormat("LoadSprite: failed to load sprite %d", index), err);
    }
