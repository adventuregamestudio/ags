...
struct ccCompiledScript : public ccScript {
    ...
    // Declare that from now on, the generated bytes stem from source line nlum.
    // The respective bytecode will not be emitted immediately because we might be
    // inmidst a bytecode.
    inline void set_line_number(int nlum) { lineno_of_next_code = nlum; }

    // If the source line has changed and the respective bytecode has not yet been
    // generated, generate it now.
    void flush_lineno();
