...
\it{NOTE:} The demo game is currently under development. It has various unfinished
...
\begin{itemize}
\item \it{DirectDraw}: any Windows-based PC with DirectX 5 or later installed.
\item \it{Direct3D}: any Windows-based PC with DirectX 9.0 installed and a graphics card designed
...
\begin{itemize}
\item \urlref{Tutorial Part 1 - Creating the game}{acintro1.htm}
\item \urlref{Tutorial Part 2 - Creating your first room}{acintro2.htm}
\item \urlref{Tutorial Part 3 - Hotspots and events}{acintro3.htm}
\item \urlref{Tutorial Part 4 - Objects}{acintro4.htm}
\item \urlref{Tutorial Part 5 - Managing inventory}{acintro5.htm}
\item \urlref{Tutorial Part 6 - Using your own graphics}{acintro6.htm}
\item \urlref{Tutorial Part 7 - Animations and cutscenes}{acintro7.htm}
\item \urlref{Tutorial Part 8 - Conversations}{acintro8.htm}
\item \urlref{Tutorial Part 9 - Cursors and fonts}{acintro9.htm}
\end{itemize}
...
\bf{IMPORTANT:} You must set up the palette as you want it before you start
...
\it{NOTE:} The transparent colour used by AGS is palette index 0 (for 256-colour
...
\subsubsection*{Tiled sprite import}
...
\subsubsection*{Alpha blended sprites}
...
\it{NOTE:} Currently, alpha blended sprites cannot be antialiased, so if you have
...
\it{NOTE:} The first frame in each loop is the standing still frame. When walking, the
...
\it{NOTE}: the blinking view is currently only supported with sierra-style speech.
...
\it{NOTE:} Do \bf{NOT} indent these lines with spaces or tabs. Indented lines
...
\subsubsection*{Using scripting commands in dialogs}
...
\item \it{Sprite alpha rendering style} -- determines which rendering method to use
  in 32-bit games when an image is drawn over \helpref{drawing surface}{DrawingSurfaceFunctions}.
...
\item \it{Resources split every Mb} -- see \helpref{here}{SplitRes} for information.
...
\item \it{Enable mouse wheel support} -- if enabled, on_mouse_click can be called with
  ...
  \it{NOTE:} Not all mice have mouse wheels, and the DOS engine does not support the
...
\item \it{Anti-alias TTF fonts} -- If enabled, any TTF fonts you have in your game will
  ...
  \it{NOTE} that anti-aliasing is not currently done on lucasarts-style speech due to
  technical reasons.
...
\item \it{Characters turn to face direction} -- if set, then when a character turns round
  with the \helprefn{Character.FaceLocation}{Character.FaceLocation} or
...
See Also: \helprefn{Enhanced Saved Games}{EnhancedSaveGames},
...
\end{itemize}
There are also some fonts available on the \urlref{AGS website}{http://www.adventuregamestudio.co.uk/fonts}.
...
\it{NOTE:} If you go to your Windows Fonts folder, you will not be able to select
...
\it{NOTE:} Font 0 is used as the normal text font, and font 1 is used as the
...
\it{NOTE:} (256-colour only) The backgrounds frames each have their own palette (unless
...
\it{NOTE:} Light levels only work when the character's graphic is at the same
colour depth as the background (ie. a 256-colour character in a hi-colour
game won't get lightened).

\it{NOTE:} In a 256-colour game, only darkening areas (light level < \verb$100%$) will work.
Also, depending on the room palette the quality of the darkening will vary
in 256-colour games.

\it{NOTE:} Light levels affect characters and objects, depending on
...
\subsection*{Audio in the Editor}
...
\subsection*{Playing audio in the game}
...
\subsection*{Priorities and channels}
...
\subsection*{Seeking and changing volume}
...
\subsection*{Using Audio Channels}
...
\subsection*{Overall system volume}
...
\it{NOTE:} WAV, OGG and MP3 format files can be used for speech.

\it{NOTE:} You cannot use speech file numbers above 9999. That is, you can
...
\it{SeeAlso:} \helprefn{Speech.VoiceMode}{Speech.VoiceMode} script property.
...
\it{NOTE:} The AUDIO.VOX file contains audio clips that you have marked as "InSeperateVOX"
...
\subsection*{When should I use a global variable?}
...
\subsection*{What about GlobalInts and Graphical Variables?}
...
\subsection*{How do I use global variables?}
...
\subsection*{What are custom properties?}
...
\subsection*{Getting values in the script}

To access the properties from the script, there are various script functions. See
their descriptions for how they work:
\begin{itemize}
\item \helprefn{Character.GetProperty}{Character.GetProperty},
\item \helprefn{Character.GetTextProperty}{Character.GetTextProperty},
\item \helprefn{Hotspot.GetProperty}{Hotspot.GetProperty},
\item \helprefn{Hotspot.GetTextProperty}{Hotspot.GetTextProperty},
\item \helprefn{InventoryItem.GetProperty}{InventoryItem.GetProperty},
\item \helprefn{InventoryItem.GetTextProperty}{InventoryItem.GetTextProperty},
\item \helprefn{Object.GetProperty}{Object.GetProperty},
\item \helprefn{Object.GetTextProperty}{Object.GetTextProperty},
\item \helprefn{GetRoomProperty}{GetRoomProperty}, and
\item \helprefn{Room.GetTextProperty}{Room.GetTextProperty}.
\end{itemize}
...
\subsection*{How to use a plugin}
...
\begin{enumerate}
\item Place a copy of the plugin files in the AGSEDIT directory (not your game folder).

\item Start the AGS Editor up, and load your game. Go to the Plugins node in the main tree.
...
\subsection*{Voice speech lip sync}
...
\it{NOTE:} This is an unofficial feature and is not currently supported. Use at
your own risk.

\it{NOTE:} The voice sync feature only supports Sierra-style speech.
...
\it{NOTE:} Voice lip sync does not work well with MP3 files. It is strongly recommended
...
\subsection*{Using downloaded templates}
...
\subsection*{Creating your own template}
...
\it{NOTE:} Do not simply make a template out of a half-finished game. If you want to make
...
\subsection*{The script debugger}
...
\it{NOTE:} The editor will allow you to place a breakpoint on any line of script.
...
\it{NOTE:} The Script Debugger is not supported on Windows 98 or Windows ME systems.
...
\subsection*{Current room information}
...
\subsubsection*{Extra features for Windows Vista and later}
...
\subsubsection*{Enabling the integration}
...
it to add \verb$-registergame$ to the end of the command line. Then, run the shortcut
...
\subsubsection*{Enabling Game Explorer support}
...
\subsubsection*{Game Explorer settings}
...
\subsubsection*{Adding the game to the Game Explorer}
...
\it{NOTE:} If you have both Enhanced Save Games and Game Explorer Integration enabled,
then the \verb$-registergame$ and \verb$-unregistergame$ commands will register/unregister
both.

\subsubsection*{Cross-Platform Support}
\begin{itemize}
\item Windows: \bf{Vista and later}
\item MS-DOS: \bf{No}
\item Linux: \bf{No}
\item MacOS: \bf{No}
\end{itemize}
...
\subsection*{What is source control?}
...
\subsection*{Which providers are supported?}
...
\subsection*{Which files does AGS put under source control?}
...
\begin{itemize}
\item \urlref{http://www.bigbluecup.com/actutor.htm}{http://www.bigbluecup.com/actutor.htm}
\item \urlref{http://www.bigbluecup.com/actutor2.htm}{http://www.bigbluecup.com/actutor2.htm}
\end{itemize}
...
\subsection*{So does this mean I have to throw out all my scripts?}
...
\subsection*{Ok, so uh... what's changed exactly?}
...
\subsection*{So I have to keep typing character[EGO] all the time? What a pain!}
...
\subsection*{Hmm, I see... so what exactly has been object-ised?}
...
\subsection*{How do I find out the new equivalents of old functions?}
...
\subsection*{Which commands haven't been changed?}
...
\subsection*{What's the deal with these "eBlock" type things?}
...
\subsection*{So do I have to pass all these things like eBlock every time I call the function?}
...
\subsection*{So what else is new?}
...
\subsection*{Is there anything else I should watch out for?}
...
\subsection*{Blimey, that's a lot to take in. Where do I start?}
...
\it{NOTE:} Some of the new functions are provided on the Game object -- for example, the new
...
\end{verbatim}
will mean that \it{text} now contains "HelloWorld".
...
\subsection*{Where do I put this code?}
...
\row{ { game.lipsync_speed }&{ Similar to Game.TextReadingSpeed, but this determines how quickly the text is 'read' out by the mouth moving.
                           You should normally only set this faster than text_speed,
                           otherwise the reading will get cut off when the text times out. Default 15. }}
...
\row{ { game.min_dialogoption_width  }&{ Minimum width of textwindow-based dialog options
                                         box. Default 0. }}
\row{ { game.narrator_speech }&{ Which character ID to use for voice speech within Display() command. Default initial player character.
                           You can also use NARRATOR which uses 'NARR' prefix - special narrator character. }}
...
\row{ { game.speech_text_align }&{ Sets how text in Lucasarts-style speech is aligned.
                                    Same possible values as game.text_align, default eAlignCentre }}
\row{ { game.speech_text_gui }&{   The textwindow GUI number used for sierra-style speech.}}
\row{ { game.text_align }&{ Sets how text in message boxes and Sierra-style speech is aligned:
                                See \helprefn{Speech.TextAlignment}{Speech.TextAlignment} for details.
                                This does not affect Lucasarts-style speech, which is always centred. }}
...
\item \it{unhandled_event (int what, int type)} -- Called when an event occurs,
  ...
  \begin{tabular}{|c|c|l|}
  \row{ {\bf{WHAT}} & {\bf{TYPE}} & {\bf{Description}} }
...
\subsection*{What's the difference between them?}
...
\subsection*{What would I use each one for?}
...
\subsection*{How do I create them?}
...
\subsection*{Can you show me an example?}
...
\it{NOTE:} This topic involves some advanced scripting. If you're just starting out with AGS,
...
\end{verbatim}
\it{Used by:} \helprefn{Mouse.IsButtonDown}{Mouse.IsButtonDown}
...
int health[50];
\end{verbatim}
This example declares 50 int variables, all called \it{health}.

You access each separate variable via its \bf{index} (the number in the brackets). Indexes
...
\begin{tabular}{|c|l|l|}
\row{ {\bf{Operator}} & {\bf{Description}} & {\bf{Example}} }
\row{ {\verb$!$}  & {NOT}                      & {\verb$if (!a)$} }
\row{ {\verb$*$}  & {Multiply}                 & {\verb$a = b * c;$} }
\row{ {\verb$/$}  & {Divide}                   & {\verb$a = b / c;$} }
\row{ {\verb$%$}  & {Remainder}                & {\verb$a = b % c;$} }
\row{ {\verb$+$}  & {Add}                      & {\verb$a = b + c;$} }
\row{ {\verb$-$}  & {Subtract}                 & {\verb$a = b - c;$} }
\row{ {\verb$<<$} & {Bitwise Left Shift}       & {\verb$a = b << c;$} }
\row{ { }         & {(advanced users only)}    & { } }
\row{ {\verb$>>$} & {Bitwise Right Shift}      & {\verb$a = b >> c;$} }
\row{ { }         & {(advanced users only)}    & { } }
\row{ {\verb$&$}  & {Bitwise AND}              & {\verb$a = b & c;$} }
\row{ { }         & {(advanced users only)}    & { } }
\row{ {\verb$|$}  & {Bitwise OR}               & {\verb$a = b | c;$} }
\row{ { }         & {(advanced users only)}    & { } }
\row{ {\verb$^$}  & {Bitwise XOR}              & {\verb$a = b ^ c;$} }
\row{ { }         & {(advanced users only)}    & { } }
\row{ {\verb$==$} & {Is equal to}              & {\verb$if (a == b)$} }
\row{ {\verb$!=$} & {Is not equal to}          & {\verb$if (a != b)$} }
\row{ {\verb$>$}  & {Is greater than}          & {\verb$if (a > b)$} }
\row{ {\verb$<$}  & {Is less than}             & {\verb$if (a < b)$} }
\row{ {\verb$>=$} & {Is greater than or equal} & {\verb$if (a >= b)$} }
\row{ {\verb$<=$} & {Is less than or equal}    & {\verb$if (a <= b)$} }
\row{ {\verb$&&$} & {Logical AND}              & {\verb$if (a && b)$} }
\row{ {\verb$||$} & {Logical OR}               & {\verb$if (a || b)$} }
\end{tabular}
...
\subsection{if, else statements}\label{ifelsestatements}\index{if statement}\index{else statement}%
...
\end{verbatim}
If \verb$<expression>$ is true, then \verb$<statements1>$ are run.
...
\end{verbatim}
If \verb$<expression>$ is true, then \verb$<statements1>$ are run like before.
If \verb$<expression>$ is not true, \verb$<statements2>$ are run instead.
...
\end{verbatim}
Runs \verb$<statements>$ continuously, while \verb$<expression>$ is true.
...
\it{NOTE:} To use optional parameters, you need to have an "import" declaration for the function
...
\it{Note:} Recommended for advanced users only.
...
\begin{enumerate}
\item \it{Accessing members of the current struct} -- When you are creating
  custom \helprefn{structs}{struct}, you use the "this" keyword inside member
...
\item \it{Declaring extender functions} -- Please see the
  \helprefn{Extender functions}{ExtenderFunctions} page for details.
\end{enumerate}
...
\it{NOTE:} You \bf{MUST} import external variables with the correct type. If \verb$counter$
was declared as a \bf{short} in the global script, you MUST import it as a short, otherwise
your game may crash.

\it{NOTE:} You cannot import old-style \verb$string$ variables (this does not
...
\it{NOTE:} The \it{noloopcheck} keyword must be placed between "function" and the function's name.
...
\it{NOTE:} If AGS gives you a script iterations error, \bf{DO NOT} just automatically
...
\it{NOTE:} MIDI music files do not support panning.
...
\it{NOTE:} This command only affects the current sound being played on the channel.
When a new audio clip starts playing on this channel, the volume will be set to the
DefaultVolume of the new audio clip.

\it{NOTE:} The volume returned by this property is the channel's base volume, ie. it does
...
\it{NOTE:} AGS can only play one MIDI file at a time.
...
Otherwise, this command behaves identically to \helprefn{AudioClip.Play}{AudioClip.Play}.
...
This command behaves identically to \helprefn{AudioClip.Play}{AudioClip.Play},
...
\it{NOTE:} This command can only be used with the player character.
...
\it{NOTE:} This command is \bf{not} intended to change the view temporarily to
...
\it{NOTE:} Any characters with the "Clickable" property set to false will not be seen
by this function.
...
\it{NOTE:} This function locks the character's view to the specified view, so
...
\it{ALIGN} can have one of the following values:
...
\it{NOTE:} You should only use this command for minor adjustments, since the offsets do not
...
\it{NOTE:} This is the only command in AGS which uses actual game-resolution co-ordinates.
...
\it{NOTE:} This function does not support Whole-Screen speech.
...
\it{NOTE:} The DELAY is actually relative to the game speed. Setting this to 1 means
...
\it{NOTE:} Due to a quirk in AGS, you cannot set the Idle View to view 1. In the unlikely event that you
...
\it{NOTE:} This function CANNOT be called while the character is moving, so
...
\it{NOTE:} This function only works in hi-colour games and with hi-colour sprites.
...
\it{NOTE:} this function only works with characters which are on the current screen.

\it{NOTE:} if you need to find out when the character has reached its destination,
...
\end{verbatim}
\begin{itemize}
\item Returns 1 if the character is currently animating.
\item Returns 0 if the character has finished its animation.
\end{itemize}
...
\it{NOTE:} This property has no effect unless the \helprefn{Solid}{Character.Solid} property
is set to \it{true}.
...
\it{NOTE:} enabling this property does not currently work properly when using
the Direct3D driver.
...
\it{NOTE:} This property is ignored if lip sync is enabled.

\it{NOTE:} This property \bf{cannot} be used if the Speech.UseGlobalSpeechAnimationDelay is set to \it{true}.
...
\it{NOTE:} Transparency only works in 16-bit and 32-bit colour games.

\it{NOTE:} When using the DirectX 5 driver, a large transparent character can significantly slow
down AGS.
...
\it{NOTE:} Do \bf{NOT} change this property while the character is moving. Make
...
\begin{itemize}
\item \helprefn{Clickable}{Character.Clickable}
\item \helprefn{DiagonalLoops}{Character.DiagonalLoops}
\item \helprefn{IgnoreLighting}{Character.IgnoreLighting}
\item \helprefn{ManualScaling}{Character.ManualScaling}
\item \helprefn{ScaleMoveSpeed}{Character.ScaleMoveSpeed}
\item \helprefn{Solid}{Character.Solid}
\item \helprefn{TurnBeforeWalking}{Character.TurnBeforeWalking}
\end{itemize}
...
\it{NOTE:} Because this accesses the real-time clock on the users' system, it is not
...
\it{NOTE:} This command does not run any dialog scripts, it simply displays
...
\it{NOTE:} The conversation will not start immediately; instead, it will be run
...
\it{NOTE:} This command only works if the image to be drawn is the same colour
depth as the surface that you are drawing onto.

\it{NOTE:} Transparency does not work in 256-colour games, or with 256-colour sprites.

\it{NOTE:} The X and Y co-ordinates given are ROOM co-ordinates, not SCREEN co-ordinates.
...
\it{NOTE:} This command is not fast enough to use repeatedly to build up an image. Only
...
\it{NOTE:} You cannot use the \it{transparency} parameter with 256-colour surfaces.

\it{NOTE:} This command can be a bit on the slow side, so don't call it from repeatedly_execute.

\it{TIP:} If you want to gradually fade in a second background, create a copy of
...
\it{NOTE:} In high-colour games, the first 32 colour numbers have a special meaning
...
\it{NOTE:} This command is relatively slow. Don't use it to try and process an entire image.
...
\it{NOTE:} The \it{x}, \it{y}, \it{width} and \it{height} parameters respect
...
\it{NOTE:} This command can be slow when using the Direct3D graphics driver.
...
\it{NOTE:} This command makes all pixels that are transparent in the source sprite
...
\begin{itemize}
\item \it{eFlipLeftToRight} -- Flips the image from left to right.
\item \it{eFlipUpsideDown} -- Flips the image from top to bottom.
\item \it{eFlipBoth} -- Flips the image from top to bottom and left to right.
\end{itemize}
...
\it{NOTE:} Resizing is a relatively slow operation, so do not attempt to resize sprites
...
\it{NOTE:} Rotating is a relatively slow operation, so do not attempt to rotate sprites
every game loop; only do it when necessary.
...
\it{NOTE:} This function only works with hi-colour sprites.
...
\it{NOTE:} You \bf{MUST} close the file with the Close function when you have finished
...
\begin{itemize}
\item \verb^$SAVEGAMEDIR$^ -- Allows you to access files in the save game directory.
\item \verb^$APPDATADIR$^ -- Allows you to write/read files to a folder on the system which
...
\it{NOTE:} Open file pointers are not persisted across save games. That is, if you open
...
\it{NOTE:} This is a static function, therefore you don't need an open File pointer
...
\it{NOTE:} this command can only read back plain text lines from text files. If you
...
\it{NOTE:} This is a static function, and thus need to be called with \verb$Game.$ in front of it. See
the example below.
...
\it{NOTE:} if you specify one of the standard slots (1-50), then AGS will rearrange the other
...
\it{NOTE:} AGS keeps a count of the number of times DisableInterface is called. Every
...
\begin{itemize}
\item \helprefn{Game.SpriteWidth}{Game.SpriteWidth} (was GP_SPRITEWIDTH)
\item \helprefn{Game.SpriteHeight}{Game.SpriteHeight} (was GP_SPRITEHEIGHT)
\item \helprefn{Game.GetLoopCountForView}{Game.GetLoopCountForView} (was GP_NUMLOOPS)
\item \helprefn{Game.GetFrameCountForLoop}{Game.GetFrameCountForLoop} (was GP_NUMFRAMES)
\item \helprefn{Game.GetRunNextSettingForLoop}{Game.GetRunNextSettingForLoop} (was GP_ISRUNNEXTLOOP)
\item \helprefn{Game.GetViewFrame}{Game.GetViewFrame} (was GP_FRAMExxx, GP_ISFRAMEFLIPPED)
\item \helprefn{Game.GUICount}{Game.GUICount} (was GP_NUMGUIS)
\item \helprefn{Room.ObjectCount}{Room.ObjectCount} (was GP_NUMOBJECTS)
\item \helprefn{Game.CharacterCount}{Game.CharacterCount} (was GP_NUMCHARACTERS)
\item \helprefn{Game.InventoryItemCount}{Game.InventoryItemCount} (was GP_NUMINVITEMS)
\end{itemize}
...
\it{NOTE:} GlobalInts are now considered obsolete. Consider using
...
\it{NOTE:} This command is obsolete, and is only provided for backwards compatibility
...
\it{NOTE:} Unlike ProcessClick, this function actually works on what the player can
see on the screen - therefore, if the co-ordinates are on a GUI, a blank string is returned.

\it{NOTE:} The co-ordinates are SCREEN co-ordinates, NOT ROOM co-ordinates. This
...
\it{NOTE:} The co-ordinates are screen co-ordinates, NOT room co-ordinates. See
...
\it{TIP:} If you add a '!' character to the start of the prompt, then a Cancel button will be
...
\it{NOTE:} The numeric keypad can have inconsistent keycodes between IsKeyPressed
...
\it{NOTE:} When the game is paused, game cycles will continue to run but
...
\it{NOTE:} GUI button animations will not be paused by this command, so that
...
\it{NOTE:} This function ignores all interfaces and acts as though the point is
...
\it{NOTE:} The range returned is inclusive - ie. if you do  Random(3);  then it
...
\it{NOTE:} The game will not be restored immediately; instead, it will be
...
\it{NOTE:} The game you run will be loaded at the same resolution and colour depth as the
current game; if you mismatch colour depths some nasty results will occur.

\it{NOTE:} Make sure that the game you want to run has a filename of 8 characters or less,
or this command will fail in the DOS engine.

\it{NOTE:} The game you want to launch must have been created with the same point-version
...
\it{NOTE:} The dialog will not be displayed immediately; instead, it will be
...
\it{NOTE:} The game will not be saved immediately; instead, it will be
...
\it{NOTE:} The screenshot will be saved to the Saved Games folder.
...
\it{NOTE:} This function overrides any specific region light levels or tints on the screen.
...
\it{NOTE:} Because the mouse cursor is repainted at the game frame rate, at very
low speeds, like 10 to 20 fps, the mouse will appear to be jumpy and not
very responsive.

\it{NOTE:} If you set the \helprefn{System.VSync}{System.VSync} property to \it{true}, the game speed will be capped
...
\it{NOTE:} The restart point cannot be set while a script is running -- therefore,
...
\begin{enumerate}
\item Relative paths (eg. "Saves"). This will create a "Saves" folder inside your game folder.
\item The special tag \verb!$MYDOCS$! which allows you to create a folder for your save games
...
\it{NOTE:} It is recommended that you do not use this function. Instead, set the "Save games
...
\it{NOTE:} the timer will not tick while the game is paused.
...
\it{NOTE:} Global Messages were a feature of AGS 2.x and are now obsolete. You will not
...
\it{NOTE:} This property is ignored if lip-sync is enabled, or if the General Settings
...
\it{NOTE:} This command will not find any GUIs that are set as Non-Clickable (ie.
...
\it{NOTE:} When using the DirectX 5 driver, a large transparent GUI can significantly slow
down AGS.
...
\begin{itemize}
\helprefn{BringToFront (inherited)}{GUIControl.BringToFront}
\helprefn{Clickable property (inherited)}{GUIControl.Clickable}
\helprefn{Enabled property (inherited)}{GUIControl.Enabled}
\helprefn{Height property (inherited)}{GUIControl.Height}
\helprefn{ID property (inherited)}{GUIControl.ID}
\helprefn{OwningGUI property (inherited)}{GUIControl.OwningGUI}
\helprefn{SendToBack (inherited)}{GUIControl.SendToBack}
\helprefn{SetPosition (inherited)}{GUIControl.SetPosition}
\helprefn{SetSize (inherited)}{GUIControl.SetSize}
\helprefn{Visible property (inherited)}{GUIControl.Visible}
\helprefn{Width property (inherited)}{GUIControl.Width}
\helprefn{X property (inherited)}{GUIControl.X}
...
\it{NOTE:} This command destroys the button's normal, pushed and mouseover images. If you
...
\it{NOTE:} This command does not support flipped view frames. Any frames marked as "Flipped"
...
\it{NOTE:} List boxes have a limit of 200 items. If you try to add more than that,
...
\end{verbatim}
\it{NOTE:} The save game list can only hold 50 save games. If ListBox.ItemCount
...
\it{NOTE:} Calling this function causes other items in the list to get re-numbered, so
...
\it{NOTE:} If the list box's "Hide Border" setting is enabled, then the scroll arrows will
...
\it{NOTE:} You must use the FillSaveGameList command in order to populate
this array.
...
\it{NOTE:} This property is only used if the "Use selected inventory graphic for cursor"
...
\it{NOTE:} For backwards compatibility, if you change this property and the CursorGraphic
...
\it{NOTE:} To change what the Use Inventory cursor looks like, you should change
...
\it{Compatibility:} \it{eMouseMiddle} supported by \bf{AGS 3.2.0} and later versions,
...
\it{NOTE:} You must call this \bf{BEFORE} you change to your new temporary cursor, or the
...
\it{NOTE:} Only use this command when absolutely necessary. Moving the mouse cursor
...
\end{itemize}
\it{NOTE:} These CD Audio functions are slow compared to all the other script
...
\begin{itemize}
\item Windows: \bf{Yes, but supports 1 CD-ROM drive only}
\item MS-DOS: \bf{Yes, if CD-ROM device driver loaded}
\item Linux: \bf{Yes, but supports 1 CD-ROM drive only}
\item MacOS: \bf{No}
\end{itemize}
...
\it{NOTE:} This function used to be called IsVoxAvailable, but has now been renamed to avoid confusion.
...
\it{NOTE:} In 256-colour games, PlayVideo is not supported. Please use a FLC/FLI video
...
\it{NOTE:} This function only performs a rectangular check, even when pixel-perfect
...
\it{NOTE:} after calling this function, you cannot use the object any more and
it is permanently removed from the game.

\it{NOTE:} objects can only be merged if the object graphic was imported at
...
\it{NOTE:} This command cannot be used while the object is moving.
...
\end{verbatim}
\begin{itemize}
\item Returns 1 if the specified object is currently animating.
\item Returns 0 if the object has finished its animation.
\end{itemize}
...
\it{NOTE:} This property has no effect unless the \helprefn{Solid}{Object.Solid} property
is set to \it{true}.
...
\it{NOTE:} solid objects only block characters, they don't block other objects from
moving through them.
...
\it{NOTE:} When using the DirectX 5 driver, a large transparent object can significantly slow
down AGS.
...
\it{NOTE:} This property cannot be changed while the object is moving.
...
\it{NOTE:} If you try to use this co-ordinate with Object.GetAtScreenXY, you will find that
...
\it{NOTE:} large overlays, in the same way as objects, can impact performance
while displayed.

\it{NOTE:} there is currently a maximum of 10 overlays displayed at any one time. Some other
...
\it{NOTE:} if the Overlay object goes out of scope, the overlay will be removed. Hence,
...
\it{NOTE:} This command only works in 256-colour games.
...
\it{NOTE:} This function will allow you to change the colours which are "locked"
...
\it{NOTE:} Unlike GetHotspotAtLocation, the co-ordinates specified are ROOM co-ordinates. This
...
\it{NOTE:} Unlike the other RunInteraction commands, this one does not take a cursor mode.
...
\it{NOTE:} This function only works in hi-colour games.
...
\it{NOTE:} If the \helprefn{Region.TintEnabled}{Region.TintEnabled} property is false, then
...
\it{NOTE:} Drawing onto the room background can be slow, especially when using the Direct3D
...
GetPlayerCharacter ()
\end{verbatim}
\bf{This command is now obsolete.}
...
\it{NOTE:} When the player leaves the screen, all the walkable areas are reset.
...
\it{NOTE:} You cannot reset the current room (ie. the room that the player is in).
...
\begin{enumerate}
\item \it{Use the same value for MIN and MAX} -- This will give the walkable area fixed
  scaling (same as setting it in the editor with "Use continuous scaling" un-ticked).
\item \it{Use different values for MIN and MAX} -- In this case, continuous scaling is
...
\it{NOTE:} The co-ordinates supplied are 320x200-scale co-ordinates, and will
be automatically multiplied up by the engine.

\it{NOTE:} This function has no effect if the current room isn't a scrolling room.
...
\it{NOTE:} This command is currently experimental, since it causes a massive
...
\it{NOTE:} This feature does not work in 256-colour games.
...
\it{NOTE:} This property only affects the animation if voice mode is disabled.
...
Enables/disables the custom speech portrait placement. When set to \it{true} the
character portraits are positioned at screen coordinates defined by
\helprefn{Speech.PortraitXOffset}{Speech.PortraitXOffset} and
\helprefn{Speech.PortraitY}{Speech.PortraitY}. When set to \it{false} the
portraits will be automatically aligned again.

\it{NOTE:} This property has no effect if the Lucas-Arts speech style is used.
...
\it{NOTE:} The property is only used when the \bf{Speech.UseGlobalSpeechAnimationDelay}
is set to \it{true}. This property \bf{cannot} be used if the global speech animation
...
\it{NOTE:}The property is only used when the \bf{Speech.CustomPortraitPlacement} is set to \it{true}.
...
\it{NOTE:} The specified key will only skip speech if the appropriate speech skip style is enabled.
...
\it{TIP:} To do a case-sensitive comparison of two strings, it's easier to just use the == operator.
...
\it{NOTE:} This function is static, which means you do not call it on
...
\it{NOTE:} This function is static, which means you do not call it on an existing
...
\it{NOTE:} The new string is returned from this function; it
...
\it{NOTE:} To convert a float to a string, you can use the \helprefn{String.Format}{String.Format}
command.
...
\it{NOTE:} To convert an integer to a string, you can use the \helprefn{String.Format}{String.Format}
command.
...
\it{NOTE:} The \it{Chars} array is read-only. If you want to change one of the characters
...
\subsubsection*{Cross-Platform Support}
\begin{itemize}
\item Windows: \bf{Direct3D driver}
\item MS-DOS: \bf{No}
\item Linux: \bf{No}
\item MacOS: \bf{No}
\end{itemize}
...
\it{NOTE:} Do \bf{NOT} use this to calculate the centre of the screen when working
...
\subsubsection*{Cross-Platform Support}
\begin{itemize}
\item Windows: \bf{Full-screen only}
\item MS-DOS: \bf{No}
\item Linux: \bf{No}
\item MacOS: \bf{No}
\end{itemize}
...
\it{NOTE:} This property has no effect with the Direct3D driver.
...
\it{NOTE:} Display is a blocking function - that is, control will not return
...
\it{NOTE:} This function is part of the Verb Coin template and is only available if
...
\it{NOTE:} This variable is part of the Verb Coin template and is only available if
you used this template to create your game.
...
There are maximum...
\begin{tabular}{|r@{ }l|}
\row{        {40} & {objects per room} }
\row{       {299} & {state-saving rooms per game} }
\row{       {300} & {inventory items} }
\row{     {30000} & {imported sprites} }
\row{        {30} & {controls on each GUI} }
\row{       {500} & {dialog topics} }
\row{        {30} & {options per dialog topic} }
\row{        {20} & {screen overlays at a time} }
\row{         {5} & {background frames per room} }
\row{        {20} & {mouse cursors} }
\row{         {8} & {audio channels} }
\row{       {100} & {local messages per room (excluding script)} }
\row{        {30} & {fonts} }
\row{ {unlimited} & {words in the text parser dictionary} }
\row{ {unlimited} & {characters} }
\row{ {unlimited} & {views} }
\row{ {unlimited} & {GUIs} }
\row{ {unlimited} & {loops per view} }
\row{ {unlimited} & {frames per loop} }
\end{tabular}
...
\begin{tabular}{|l|l|}
\row{ {F1}             & {Help} }
\row{ {F2}             & {Game Statistics} }
\row{ {F3}             & {Find Next} }
\row{ {Ctrl+F4}        & {lose tab} }
\row{ {F5}             & {Run with Debugger} }
\row{ {Ctrl+F5}        & {un without Debugger} }
\row{ {F7}             & {Build EXE} }
\row{ {F9}             & {Toggle Breakpoint} }
\row{ {F11}            & {Step Into} }
\row{ {}               & {} }
\row{ {Ctrl+A}         & {Select All} }
\row{ {Ctrl+B}         & {Match Brace} }
\row{ {Ctrl+C}         & {Copy} }
\row{ {Ctrl+D}         & {Duplicate line to next} }
\row{ {Ctrl+E}         & {Replace} }
\row{ {Ctrl+F}         & {Find} }
\row{ {Ctrl+G}         & {Open GlobalScript.asc} }
\row{ {Ctrl+H}         & {Open GlobalScript.ash} }
\row{ {Ctrl+L}         & {Open Game} }
\row{ {Ctrl+Q}         & {Quit} }
\row{ {Ctrl+R}         & {Save Room} }
\row{ {Ctrl+S}         & {Save Game} }
\row{ {Ctrl+V}         & {Paste} }
...
\row{ {Ctrl+Z}         & {Undo} }
\row{ {}               & {} }
\row{ {Ctrl+Space}     & {Show Autocomplete} }
\row{ {Ctrl+Tab}       & {Next tab} }
\row{ {Ctrl+Shift+Tab} & {Previous tab} }
\row{ {}               & {} }
\row{ {Tab}            & {Indent selected lines} }
\row{ {Shift+Tab}      & {Un-indent selected lines} }
\end{tabular}
...
\begin{tabular}{|l|l|r|}
\row{ {\bf{AGS KeyCode}}     & {\bf{Key}}          & {\bf{ASCII code}} }
\row{ {eKeyNone}             & {\verb|none|}       &   {0} }
\row{ {eKeyCtrlA}            & {\verb|Ctrl+A|}     &   {1} }
\row{ {eKeyCtrlB}            & {\verb|Ctrl+B|}     &   {2} }
\row{ {eKeyCtrlC}            & {\verb|Ctrl+C|}     &   {3} }
\row{ {eKeyCtrlD}            & {\verb|Ctrl+D|}     &   {4} }
\row{ {eKeyCtrlE}            & {\verb|Ctrl+E|}     &   {5} }
\row{ {eKeyCtrlF}            & {\verb|Ctrl+F|}     &   {6} }
\row{ {eKeyCtrlG}            & {\verb|Ctrl+G|}     &   {7} }
\row{ {eKeyCtrlH}            & {\verb|Ctrl+H|}     &   {8} }
\row{ {eKeyCtrlI}            & {\verb|Ctrl+I|}     &   {9} }
\row{ {eKeyCtrlJ}            & {\verb|Ctrl+J|}     &  {10} }
\row{ {eKeyCtrlK}            & {\verb|Ctrl+K|}     &  {11} }
\row{ {eKeyCtrlL}            & {\verb|Ctrl+L|}     &  {12} }
\row{ {eKeyCtrlM}            & {\verb|Ctrl+M|}     &  {13} }
\row{ {eKeyCtrlN}            & {\verb|Ctrl+N|}     &  {14} }
\row{ {eKeyCtrlO}            & {\verb|Ctrl+O|}     &  {15} }
\row{ {eKeyCtrlP}            & {\verb|Ctrl+P|}     &  {16} }
\row{ {eKeyCtrlQ}            & {\verb|Ctrl+Q|}     &  {17} }
\row{ {eKeyCtrlR}            & {\verb|Ctrl+R|}     &  {18} }
\row{ {eKeyCtrlS}            & {\verb|Ctrl+S|}     &  {19} }
\row{ {eKeyCtrlT}            & {\verb|Ctrl+T|}     &  {20} }
\row{ {eKeyCtrlU}            & {\verb|Ctrl+U|}     &  {21} }
\row{ {eKeyCtrlV}            & {\verb|Ctrl+V|}     &  {22} }
\row{ {eKeyCtrlW}            & {\verb|Ctrl+W|}     &  {23} }
\row{ {eKeyCtrlX}            & {\verb|Ctrl+X|}     &  {24} }
\row{ {eKeyCtrlY}            & {\verb|Ctrl+Y|}     &  {25} }
\row{ {eKeyCtrlZ}            & {\verb|Ctrl+Z|}     &  {26} }
\row{ {eKey0}                & {\verb|0|}          &  {48} }
\row{ {eKey1}                & {\verb|1|}          &  {49} }
\row{ {eKey2}                & {\verb|2|}          &  {50} }
\row{ {eKey3}                & {\verb|3|}          &  {51} }
...
\row{ {eKeyY}                & {\verb|Y|}          &  {89} }
\row{ {eKeyZ}                & {\verb|Z|}          &  {90} }
\row{ {eKeyAmpersand}        & {\verb|&|}          &  {38} }
\row{ {eKeyAsterisk}         & {\verb|*|}          &  {42} }
\row{ {eKeyAt}               & {\verb|@|}          &  {64} }
\row{ {eKeyBackSlash}        & {\verb|\|}          &  {92} }
\row{ {eKeyBackspace}        & {\verb|Backspace|}  &   {8} }
\row{ {eKeyCloseBracket}     & {\verb|]|}          &  {93} }
\row{ {eKeyCloseParenthesis} & {\verb|)|}          &  {41} }
\row{ {eKeyColon}            & {\verb|:|}          &  {58} }
\row{ {eKeyComma}            & {\verb|,|}          &  {44} }
\row{ {eKeyDelete}           & {\verb|Delete|}     & {383} }
\row{ {eKeyDollar}           & {\verb|$|}          &  {36} }
\row{ {eKeyDoubleQuote}      & {\verb|"|}          &  {34} }
\row{ {eKeyEquals}           & {\verb|=|}          &  {61} }
\row{ {eKeyEscape}           & {\verb|ESC|}        &  {27} }
\row{ {eKeyExclamationMark}  & {\verb|!|}          &  {33} }
\row{ {eKeyForwardSlash}     & {\verb|/|}          &  {47} }
\row{ {eKeyGreaterThan}      & {\verb|>|}          &  {62} }
\row{ {eKeyHash}             & {\verb|#|}          &  {35} }
\row{ {eKeyHyphen}           & {\verb|-|}          &  {45} }
\row{ {eKeyInsert}           & {\verb|Insert|}     & {382} }
\row{ {eKeyLessThan}         & {\verb|<|}          &  {60} }
\row{ {eKeyOpenBracket}      & {\verb|[|}          &  {91} }
\row{ {eKeyOpenParenthesis}  & {\verb|(|}          &  {40} }
\row{ {eKeyPercent}          & {\verb|%|}          &  {37} }
\row{ {eKeyPeriod}           & {\verb|.|}          &  {46} }
\row{ {eKeyPlus}             & {\verb|+|}          &  {43} }
\row{ {eKeyQuestionMark}     & {\verb|?|}          &  {63} }
\row{ {eKeyReturn}           & {\verb|RETURN|}     &  {13} }
\row{ {eKeySemiColon}        & {\verb|;|}          &  {59} }
\row{ {eKeySingleQuote}      & {\verb|'|}          &  {39} }
\row{ {eKeySpace}            & {\verb|SPACE|}      &  {32} }
\row{ {eKeyTab}              & {\verb|TAB|}        &   {9} }
\row{ {eKeyUnderscore}       & {\verb|_|}          &  {95} }
\row{ {eKeyF1}               & {\verb|F1|}         & {359} }
\row{ {eKeyF2}               & {\verb|F2|}         & {360} }
\row{ {eKeyF3}               & {\verb|F3|}         & {361} }
\row{ {eKeyF4}               & {\verb|F4|}         & {362} }
\row{ {eKeyF5}               & {\verb|F5|}         & {363} }
\row{ {eKeyF6}               & {\verb|F6|}         & {364} }
\row{ {eKeyF7}               & {\verb|F7|}         & {365} }
\row{ {eKeyF8}               & {\verb|F8|}         & {366} }
\row{ {eKeyF9}               & {\verb|F9|}         & {367} }
\row{ {eKeyF10}              & {\verb|F10|}        & {368} }
\row{ {eKeyF11}              & {\verb|F11|}        & {433} }
\row{ {eKeyF12}              & {\verb|F12|}        & {434} }
\row{ {eKeyHome}             & {\verb|Home|}       & {371} }
\row{ {eKeyUpArrow}          & {\verb|UpArrow|}    & {372} }
\row{ {eKeyPageUp}           & {\verb|PageUp|}     & {373} }
\row{ {eKeyLeftArrow}        & {\verb|LeftArrow|}  & {375} }
\row{ {eKeyNumPad5}          & {\verb|NumPad 5|}   & {376} }
\row{ {eKeyRightArrow}       & {\verb|RightArrow|} & {377} }
\row{ {eKeyEnd}              & {\verb|End|}        & {379} }
\row{ {eKeyDownArrow}        & {\verb|DownArrow|}  & {380} }
\row{ {eKeyPageDown}         & {\verb|PageDown|}   & {381} }
\end{tabular}
...
\section*{Q. What's the deal with the license? What does it mean in plain English?}
...
\section*{Q. Why the swapware license? Why aren't you charging for it?}
...
\section*{Q. When I enter a certain room, I just get a black screen.}
...
\begin{enumerate}
\item \it{Concurrency issues} -- that is, while a blocking script was running in 2.72
...
\item \it{Blocking interactions} -- in 2.72, a "Run Dialog" interaction, for example,
...
\section*{Saving and testing your game}
...
\it{NOTE:} When you use the Run command, your game EXE file does \bf{NOT} get built.
...
\section*{RawDraw scripting changes}
...
\begin{enumerate}
\item \verb$new$ is now a reserved word. This means that if you had any variables
  called "new" then your script will fail to compile. Just rename the variable
  to something else.
\item Because of the removal of some system limits, some of the AGS_MAX_ constants
...
\section*{What are native hi-res co-ordinates?}
...
\section*{Setup has changed, I can't select the resolution any more!}
...
\section*{Why a new audio system?}
...
\section*{So how do I name my audio files?}
...
\section*{There is now an AudioCache folder, do I still need the Sound and Music folders?}
...
\section*{What about controlling the volume?}
...
\section*{Wait, what's an audio channel?}
...
\section*{PlayAmbientSound is obsolete! How do I do ambient sounds?}
...
\section*{Is there any new cool stuff that I can do?}
...
\section*{Has voice speech changed?}
...
\section*{Where should I look for more info?}
...
\section*{Game-wide speech animation delay}
...
\it{NOTE:} when older projects are imported, it is set to False automatically.
...
\section*{What information does it send?}
...
\section*{Why should I enable this?}
...
\section*{When does it send this information?}

The editor contacts the AGS website once a month and sends it these details.

\section*{Will games that I make contact the AGS website?}
...
\section*{AGS Demo Game II Credits:}
