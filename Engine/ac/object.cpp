...
void ValidateViewAnimVLF(const char *apiname, int view, int loop, int &sframe)
{
    if ((view < 1) || (view > game.numviews))
        quitprintf("!%s: invalid view (range is 1..%d.", apiname, view + 1, game.numviews);
    if (views[view].numLoops == 0)
        quitprintf("!%s: view %d does not have any loops.", apiname, view + 1);
    if (loop < 0 || loop >= views[view].numLoops)
        quitprintf("!%s: invalid loop number for view %d (range is 0..%d).", apiname, view + 1, views[view].numLoops);
    ...
    if (views[view].loops[loop].numFrames < 1)
        ...
        debug_script_warn("%s: invalid starting frame number for view %d loop %d (range is 0..%d)",
            view + 1, loop, views[view].loops[loop].numFrames);
