...
// Simplified game data representation for headless compilation.
struct GameData
{
    ...
    std::string custom_data_dir;       // comma-separated folders (Settings/CustomDataDir)
    int sprite_file_compression = 0;   // 0=none, 1=RLE, 2=LZW, 3=Deflate
    bool optimize_sprite_storage = true;
    ...
    // Sprite source metadata (from AGF XML <Source> element)
    ...
    // Pending sprite metadata from AGF parsing, applied in InitSubsystems.
    ...
    struct AudioClipInfo {
        int id;
        int fixed_index = 0;          // stable Index for AudioCache/auXXXXXX.ext
        std::string name;
        std::string filename;
