...
int read_variable_into_ax(ccCompiledScript * scrip, ags::SymbolScript syml, int syml_len, int mustBeWritable = 0, bool negateLiteral = false);
int do_variable_memory_access(
    ccCompiledScript *scrip, ags::Symbol variableSym,
    int variableSymType, bool isProperty,
    bool writing, int mustBeWritable,
...
bool is_type_mismatch_oneway(int typeIs, int typeWantsToBe)
{
    ...
    if ((typeIs & STYPE_POINTER) || (typeWantsToBe & STYPE_POINTER))
    {
        ...
        // Types cannot be identical here, but check against inherited classes
...
int parse_subexpr_NoOps(ccCompiledScript * scrip, ags::SymbolScript symlist, size_t symlist_len)
{
    ...
    if (symlist_len == 1)
    {
        // Must be a variable, otherwise it's invalid
        int retval = read_variable_into_ax(scrip, symlist, 1);
        if (retval < 0) return retval;
        return 0;
    }
...
int parseArrayIndexOffsets(ccCompiledScript *scrip, VariableSymlist *thisClause, bool writingOperation, bool *isArrayOffset) {
    ...
    {
        // there aren't any array brackets, so nothing to do
        return 0;
    }
...
inline int do_variable_ax_PrepareComponentAccess_MemberFunction(bool isLastClause, bool & getAddressOnlyIntoAX, bool & doMemoryAccessNow)
{
...
// We access a component of a struct in order to read or write it. 
// This is a property.
int do_variable_ax_PrepareComponentAccess_Property(ccCompiledScript * scrip, ags::Symbol variableSym, VariableSymlist * thisClause, bool writing, bool writingThisTime, int mustBeWritable, bool & getAddressOnlyIntoAX, bool & doMemoryAccessNow, bool & isProperty, bool & isArrayOffset, bool & readonly_cannot_cause_error)
{
...
int do_variable_ax_PrepareComponentAccess_AddressOnlyCases(ags::Symbol variableSym, VariableSymlist * thisClause, bool isLastClause, bool & getAddressOnlyIntoAX, bool & cannotAssign)
{
...
int do_variable_ax_PrepareComponentAccess(ags::Symbol variableSym, int variableSymType, int &currentComponentOffset, bool &getAddressOnlyIntoAX, bool &doMemoryAccessNow, bool isLastClause, VariableSymlist * thisClause, ccCompiledScript * scrip, bool writing, bool &isProperty, bool &isArrayOffset, int mustBeWritable, bool &readonly_cannot_cause_error, bool writingThisTime, bool &isDynamicArray, bool &isPointer, ags::Symbol firstVariableType, ags::Symbol firstVariableSym, bool &accessActualPointer, bool &cannotAssign)
{
    ...
    if (((variableSymType == SYM_GLOBALVAR) ||
        ...
        ((sym.entries[variableSym].flags & (SFLG_PROPERTY | SFLG_IMPORTED)) != 0))
    {
        do_variable_ax_PrepareComponentAccess_Elementary(variableSym, currentComponentOffset);
    }
    else if (variableSymType == SYM_FUNCTION)
    {
        int retval = do_variable_ax_PrepareComponentAccess_MemberFunction(isLastClause, getAddressOnlyIntoAX, doMemoryAccessNow);
        if (retval < 0) return retval;
    }
    else if (sym.entries[variableSym].flags & SFLG_PROPERTY)
    {
        int retval = do_variable_ax_PrepareComponentAccess_Property(scrip, variableSym, thisClause, writing, writingThisTime, mustBeWritable, getAddressOnlyIntoAX, doMemoryAccessNow, isProperty, isArrayOffset, readonly_cannot_cause_error);
        if (retval < 0) return retval;
    }
    else if (sym.entries[variableSym].flags & SFLG_POINTER)
    {
        int retval = do_variable_ax_PrepareComponentAccess_Pointer(scrip, variableSym, thisClause, currentComponentOffset, isDynamicArray, writing, firstVariableType, firstVariableSym, isLastClause, isArrayOffset, getAddressOnlyIntoAX, isPointer, currentComponentOffset, accessActualPointer, doMemoryAccessNow);
        if (retval < 0) return retval;
    }
    else
    {
        if (sym.entries[variableSym].flags & SFLG_DYNAMICARRAY) isDynamicArray = true;

        // only parse array index offsets if there
        int retval = parseArrayIndexOffsets(scrip, thisClause, writing, &isArrayOffset);
        if (retval < 0) return retval;
    }

    int retval = do_variable_ax_PrepareComponentAccess_AddressOnlyCases(variableSym, thisClause, isLastClause, getAddressOnlyIntoAX, cannotAssign);
    if (retval < 0) return retval;
    return 0;
}

int do_variable_ax_ActualMemoryAccess(ccCompiledScript * scrip, bool &isPointer, bool writing, bool writingThisTime, ags::Symbol variableSym, int variableSymType, bool isProperty, int mustBeWritable, bool getAddressOnlyIntoAX, bool &isArrayOffset, int &currentComponentOffset, bool accessActualPointer, ags::Symbol firstVariableSym, ags::Symbol &firstVariableType, bool &isDynamicArray, bool negateLiteral, bool isLastClause, VariableSymlist  variablePath[], size_t vp_idx)
{
...
int do_variable_ax_CheckAccess(bool writing, int mustBeWritable, bool readonly_cannot_cause_error, ags::Symbol variableSym, bool isLastClause, const size_t &vp_idx, VariableSymlist  variablePath[10], bool cannotAssign)
{
...
int do_variable_ax(ccCompiledScript*scrip, ags::SymbolScript syml, int syml_len, bool writing, int mustBeWritable, bool negateLiteral = false)
{
    ...
    for (size_t vp_idx = 0; vp_idx < variablePathSize; vp_idx++)
    {
        ...
        int retval = do_variable_ax_PrepareComponentAccess(variableSym, variableSymType, currentComponentOffset, getAddressOnlyIntoAX, doMemoryAccessNow, isLastClause, thisClause, scrip, writing, isProperty, isArrayOffset, mustBeWritable, readonly_cannot_cause_error, writingThisTime, isDynamicArray, isPointer, firstVariableType, firstVariableSym, accessActualPointer, cannotAssign);
        if (retval < 0) return retval;

        retval = do_variable_ax_CheckAccess(writing, mustBeWritable, readonly_cannot_cause_error, variableSym, isLastClause, vp_idx, variablePath, cannotAssign);
        if (retval < 0) return retval;

        if (!(doMemoryAccessNow) && !(isLastClause)) continue;

        retval = do_variable_ax_ActualMemoryAccess(scrip, isPointer, writing, writingThisTime, variableSym, variableSymType, isProperty, mustBeWritable, getAddressOnlyIntoAX, isArrayOffset, currentComponentOffset, accessActualPointer, firstVariableSym, firstVariableType, isDynamicArray, negateLiteral, isLastClause, variablePath, vp_idx);
        if (retval < 0) return retval;
...
int read_variable_into_ax(ccCompiledScript*scrip, ags::SymbolScript syml, int syml_len, int mustBeWritable, bool negateLiteral) {
...
int do_variable_memory_access_LitOrConst(ccCompiledScript * scrip, int mainVariableType, ags::Symbol variableSym, bool writing, int mustBeWritable, bool negateLiteral, int &gotValType)
{
...
int do_variable_memory_access_LitFloat(ccCompiledScript * scrip, ags::Symbol variableSym, bool writing, int mustBeWritable, int &gotValType)
{
    if ((writing) || (mustBeWritable))
    {
        cc_error("cannot write to a literal value");
...
int do_variable_memory_access_ActualAccess(ccCompiledScript * scrip, ags::Symbol mainVariableSym, int mainVariableType, ags::Symbol variableSym, bool writing, int mustBeWritable, bool negateLiteral, bool isPointer, bool &wholePointerAccess, bool addressof, int soffset, bool extraoffset, bool isDynamicArray, bool isProperty, int &gotValType)
{
    ...
    bool isDynamicArray, bool negateLiteral)
{
    ...
    int retval = do_variable_memory_access_ActualAccess(scrip, mainVariableSym, mainVariableType, variableSym, writing, mustBeWritable, negateLiteral, isPointer, wholePointerAccess, addressof, soffset, extraoffset, isDynamicArray, isProperty, gotValType);
    if (retval < 0) return retval;
...
int evaluate_for(ccInternalList * targ, ccCompiledScript * scrip, size_t &nested_level, char * nested_type, long * nested_start, std::vector<ccChunk> * nested_chunk, long * nested_info, ags::Symbol &cursym, const ags::SymbolScript &vnlist, size_t &vnlist_len, int &offset_of_funcname, char is_protected, char is_static, char is_readonly)
{
    ...
    if (sym.get_type(cursym) != SYM_SEMICOLON)
    {
        ...
        if (sym.get_type(cursym) == SYM_VARTYPE)
        {
            ...
            if (evaluate_assignment(targ, scrip, cursym, false, vnlist, vnlist_len, false))
                return -1;
    }
    ...
    if (sym.get_type(targ->peeknext()) != SYM_SEMICOLON)
    {
        ...
        hasLimitCheck = true;
        if (evaluate_expression(targ, scrip, false))
            return -1;
        if (sym.get_type(targ->peeknext()) != SYM_SEMICOLON)
        {
            cc_error("expected ';'");
    ...
    if (sym.get_type(cursym) != SYM_CLOSEPARENTHESIS)
    {
        ...
        if (evaluate_assignment(targ, scrip, cursym, true, vnlist, vnlist_len, true))
            return -1;
    } // Allow for loops without increments
    INC_NESTED_LEVEL;
    yank_chunk(scrip, nested_chunk, assignaddr, pre_fixup_count);
    if (!hasLimitCheck)
...
int evaluate_casedefault(ccInternalList * targ, ccCompiledScript * scrip, ags::Symbol cursym, char * nested_type, size_t &nested_level, int32_t * nested_assign_addr, long * nested_info, std::vector<ccChunk> * nested_chunk)
{
    ...
    else
    {
        ...
        if (evaluate_expression(targ, scrip, false)) // case n: label expression, result is in AX
            return -1;
        if (check_type_mismatch(scrip->ax_val_type, nested_info[nested_level], false))
            return -1;
        if (get_operator_valid_for_type(scrip->ax_val_type, nested_info[nested_level], vcpuOperator))
            return -1;
