...
char ccCopyright[]="ScriptCompiler32 v" SCOM_VERSIONSTR " (c) 2000-2007 Chris Jones and 2011-2026 others";
...
int cc_tokenize(const char*inpl, ccInternalList*targ, ccCompiledScript*scrip) {
    ...
    while (!fmem_eof(iii)) {
        ...
        if ((thissymbol[0] == '\'') && (thissymbol.back() == '\'')) {
            int chr = 0;
            if (thissymbol[1] == '\\') {
                chr = GetEscapedChar(thissymbol[2]);
...
int process_function_declaration(ccInternalList &targ, ccCompiledScript*scrip,
  ...
  while (1) {
    ...
    if (next_type == SYM_CLOSEPARENTHESIS) break;
      ...
      sym.entries[funcsym].funcparams[arg_idx].Name = {};
