...
\helprefn{Object.FaceDirection}{Object.FaceDirection}
...
or even a function that returns apple:
...
See Also: \helprefn{Pointers in AGS}{Pointers}


\subsection{new}\label{newstatement}%

\it{pointer variable} = \bf{new} \it{managed type};

Creates a new dynamic (managed) object of \it{managed type} and assigns it to \it{pointer variable}.

For more information on creating and using dynamic objects see:
\helprefn{Pointers in AGS}{Pointers}
\helprefn{managed (struct)}{managedmodifier}
...
\subsection{Speed property}\label{AudioChannel.Speed}\index{AudioChannel.Speed}%
...
\end{verbatim}
Runs the currently selection dialog option, the one set by \helprefn{DialogOptionsRenderingInfo.ActiveOptionID}{DialogOptionsRenderingInfo.ActiveOptionID},
and returns \bf{true} on success.
...
\it{See Also:} \helprefn{Dialog.ActiveOptionID}{Dialog.ActiveOptionID}
...
\it{See Also:} \helprefn{Dialog.RunActiveOption}{Dialog.RunActiveOption},
\helprefn{Dialog.GetOptionState}{Dialog.GetOptionState}
...
eSeekBegin - counts \it{offset} bytes starting from the file's beginning; \it{offset} must be positive.
...
\begin{verbatim}
static bool IsPluginLoaded(const string name)
\end{verbatim}
