...
See Also: \helprefn{Default setup}{Game setup}
...
\subsection{General settings}\index{settings}\index{options}\index{Anti-aliasing fonts}\index{Crossfading music}%
...
\subsection{Default setup}\index{Default setup}\index{Game setup}%
...
Most of the options here correspond to the ones you may find in the setup utility. For their meaning please refer to \section{related topic}\index{Setup}.
...
\bf{Enviroment}
\begin{itemize}
\item \bf{Custom game saves path} - defines where game will store its saves and individual user's files (e.g. achievements). This path will be used to substitute default value of \verb^$SAVEGAMEDIR$^ token in scripts. Note that when being set up in the Editor, this option only accepts relative paths (when used they will be relative to the game's location). This has to be done this way to prevent setting absolute path that does not exist on the player's machine, so typically you have to set this if you want to have saves stored locally within the game's folder. Also players will be able to modify this path in setup program (where they are actually allowed to put absolute paths too).
\item \bf{Custom shared data path} - defines where game will store its shared data files (e.g. hiscore tables). This path will be used to substitute default value of \verb^$APPDATADIR$^ token in scripts. Note that this option only accepts relative paths, so typically you have to set this if you want to have data files created locally within the game's folder. Players \bg{won't} normally be able to change it in setup program.
\end{itemize}
...
See also: \helprefn{Run-time engine setup}{Setup}
...
\bf{KNOWN ISSUES:}
\begin{itemize}\itemsep=0pt
\item Since the final release of AGS 3.4.0 usage of strings in the switch is broken:
instead of comparing by value, as intended, they are instead compared as pointers.
\item Local function variables do not work when being used in the "case" expression.
\end{itemize}
...
For detailed information about Default Setup page refer to: \helprefn{Default setup}{Game setup}.
