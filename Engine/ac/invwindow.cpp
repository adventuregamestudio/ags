...
void InventoryScreen::Prepare()
{
    ...
    top_item = 0;
    num_visible_items = 0;
    MAX_ITEMAREA_HEIGHT = ((play.GetUIViewport().GetHeight() - BUTTONAREAHEIGHT) - 20);
    in_inv_screen++;
...
#define BUTTONWID std::max(1, game.SpriteInfos[btn_select_sprite].Width)
    ...
    if (top_item + num_visible_items < numitems)
        arrowblock->FlipBlt(arrowblock, windowwid-ARROWBUTTONWID, buttonyp + 4 + ARROWBUTTONWID, Common::kFlip_Vertical);
    delete arrowblock;
}
