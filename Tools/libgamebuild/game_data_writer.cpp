// AGS - Game Data Writer (ac2game.dta)
...
#include "game_data.h"
#include "build_system.h"   // for BuildResult
...
static const int32_t GAME_DATA_VERSION_CURRENT = kGameVersion_Current;
static const char *const ENGINE_VERSION = ACI_VERSION_STR;

static const int32_t GUI_VERSION = kGuiVersion_Current;
...
static int GetViewCount(const GameData &gd);
                                      ...
                                      soff_t& ext_off_pos)
{
    ...
    // numviews
    out->WriteInt32((int32_t)GetViewCount(gd));
    // numcharacters
...
// -------------------------------------------------------------------------
static uint8_t SpriteFlagsFromMetadata(const std::string &resolution, bool alpha_channel)
{
    uint8_t f = 0;
    if (!resolution.empty() && resolution != "Real")
    {
        f |= SPF_VAR_RESOLUTION;
...
static void WriteSpriteFlags(Stream* out, const GameData& gd)
{
    ...
    for (const auto& kv : gd.pending_sprite_alpha_)
        if (kv.first > topmost) topmost = kv.first;
    for (const auto& kv : gd.pending_sprite_resolution_)
        if (kv.first > topmost) topmost = kv.first;
    ...
    const int count = topmost + 1;
    out->WriteInt32(count);

    std::vector<uint8_t> flags(count, 0);
    for (const auto& s : gd.sprites) {
        if (s.id >= 0 && s.id < count)
            flags[s.id] = SpriteFlagsFromMetadata(s.resolution, s.alpha_channel);
    }
    for (const auto& kv : gd.pending_sprite_resolution_) {
        if (kv.first >= 0 && kv.first < count) {
            auto it = gd.pending_sprite_alpha_.find(kv.first);
            const bool alpha = (it != gd.pending_sprite_alpha_.end()) ? it->second : false;
            flags[kv.first] = SpriteFlagsFromMetadata(kv.second, alpha);
        }
    }
    for (const auto& kv : gd.pending_sprite_alpha_) {
        if (kv.first >= 0 && kv.first < count && flags[kv.first] == 0 && kv.second)
            flags[kv.first] |= SPF_ALPHACHANNEL;
    }
...
static void WriteView(Stream *out, const GameData::ViewInfo &v)
{
    ...
    for (const auto &loop : v.loops)
    {
        ...
        for (const auto &frame : loop.frames)
        {
            ...
            // Editor currently writes zero offsets in compiled data
            out->WriteInt16(0);
            out->WriteInt16(0);
...
{
    const int view_count = GetViewCount(gd);
    for (int view_id = 1; view_id <= view_count; ++view_id)
    {
        const GameData::ViewInfo *view = nullptr;
        for (const auto &v : gd.views)
        {
            if (v.id == view_id)
            {
        ...
        if (!view)
            out->WriteInt16(0); // empty view slot
        else
            WriteView(out, *view);
    }
}
