...
{
    Character_StopMovingEx(chi, chi->is_moving() && (!get_movelist(chi->get_movelist_id())->IsDirectMove()));
}
    ...
    const RunPathParams &run_params)
{
    ...
    if (path)
    {
        path_result = Pathfinding::CalculateMoveList(new_mlist, *path, move_speed_x, move_speed_y, run_params);
    }
    else
    {
        MaskRouteFinder *pathfind = get_room_pathfinder();
        pathfind->SetWalkableArea(prepare_walkable_areas(chac), thisroom.MaskResolution);
        path_result = Pathfinding::FindRoute(new_mlist, pathfind, chin->x, chin->y, tox, toy, move_speed_x, move_speed_y, false, ignwal, run_params);
    }
...
bool is_char_walking_ndirect(CharacterInfo *chi)
{
    return chi->is_moving_not_turning() &&
        (!get_movelist(chi->get_movelist_id())->IsDirectMove());
}
