...
void TextEditor::AddIdentifier(const std::string& aName, const std::string& aDeclaration)
{
	Identifier id;
	id.mDeclaration = aDeclaration;
	mLanguageDefinition.mIdentifiers[aName] = id;
}

void TextEditor::ClearCustomIdentifiers()
{
	...
	if (ImGui::IsWindowHovered())
	{
		if (!shift && !alt)
		{
			...
			*/
			...
			{
				// Check if click is in the gutter (line number area) for breakpoint toggle
				ImVec2 origin = ImGui::GetCursorScreenPos();
				ImVec2 mousePos = ImGui::GetMousePos();
				float localX = mousePos.x - origin.x;
				if (localX < mTextStart)
				{
					float localY = mousePos.y - origin.y;
					int clickedLine = std::max(0, (int)floor(localY / mCharAdvance.y));
					if (clickedLine >= 0 && clickedLine < (int)mLines.size())
						ToggleBreakpoint(clickedLine + 1); // 1-based
				}
				else
				{
					mState.mCursorPosition = mInteractiveStart = mInteractiveEnd = ScreenPosToCoordinates(ImGui::GetMousePos());
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			if (mBreakpoints.count(lineNo + 1) != 0)
			{
				...
				// Draw red circle indicator in the gutter
				float radius = mCharAdvance.y * 0.35f;
				float cx = lineStartScreenPos.x + scrollX + mLeftMargin * 0.5f + radius;
				float cy = lineStartScreenPos.y + mCharAdvance.y * 0.5f;
				drawList->AddCircleFilled(ImVec2(cx, cy), radius, 0xff0000ff); // red
				drawList->AddCircle(ImVec2(cx, cy), radius, 0xff000080, 0, 1.5f); // dark red outline
			}

			// Draw execution point marker (yellow arrow) when paused in debugger
			if (mExecutionLine > 0 && (lineNo + 1) == mExecutionLine)
			{
				auto end = ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
				// Yellow highlight on the execution line
				drawList->AddRectFilled(start, end, IM_COL32(255, 255, 0, 40));

				// Draw yellow arrow in the gutter
				float arrowSize = mCharAdvance.y * 0.35f;
				float cx = lineStartScreenPos.x + scrollX + mLeftMargin * 0.5f + arrowSize;
				float cy = lineStartScreenPos.y + mCharAdvance.y * 0.5f;
				ImVec2 p1(cx - arrowSize, cy - arrowSize * 0.7f);  // top-left
				ImVec2 p2(cx - arrowSize, cy + arrowSize * 0.7f);  // bottom-left
				ImVec2 p3(cx + arrowSize * 0.5f, cy);               // right tip
				drawList->AddTriangleFilled(p1, p2, p3, IM_COL32(255, 220, 0, 255)); // yellow
				drawList->AddTriangle(p1, p2, p3, IM_COL32(180, 150, 0, 255), 1.5f); // dark outline
			}
...
{
	if (aLine < 1 || aLine > (int)mLines.size())
		return;
	if (mBreakpoints.count(aLine))
