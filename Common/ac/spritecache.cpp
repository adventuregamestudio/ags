...
void SpriteCache::removeOldest()
{
  ...
  if (liststart == listend)
  {
    ...
    {
      char msgg[150];
      sprintf(msgg, "!!!! SPRITE CACHE ERROR: Sprite cache should be empty, but still has %li bytes", cachesize);
      write_log(msgg);
    }
    mrulist[liststart] = 0;
    liststart = -1;
...
int SpriteCache::saveToFile(const char *filnam, int lastElement, bool compressOutput)
{
  ...
  spindex_out->WriteArrayOfIntPtr32((intptr_t*)&spriteoffs[0], numsprits);
  delete spindex_out;
