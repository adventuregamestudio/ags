...
namespace Pathfinding
{
    ...
    bool CalculateMoveList(MoveList &mls, const std::vector<Point> &path, float move_speed_x, float move_speed_y, const RunPathParams &run_params);
    // Calculate the MoveList from the given navigation path and variable move speeds per section.
    bool CalculateMoveList(MoveList &mls, const std::vector<Point> &path, const std::vector<Pointf> &move_speeds, const RunPathParams &run_params);
    // Append a waypoint to the move list, skip pathfinding
    bool AddWaypointDirect(MoveList &mls, int x, int y, float move_speed_x, float move_speed_y);
