...
static bool CalculateMoveList(MoveList &mls, const std::vector<Point> &path,
    float move_speed_x, float move_speed_y,
    const std::vector<Pointf> &move_speeds, const RunPathParams & run_params)
{
    MoveList mlist;
    mlist.pos = path;
                       ...
                       const RunPathParams &run_params)
{
    ...
    return CalculateMoveList(mls, path, move_speed_x, move_speed_y, std::vector<Pointf>(), run_params);
}
    ...
    const std::vector<Pointf> &move_speeds, const RunPathParams & run_params)
{
    ...
    return CalculateMoveList(mls, path, 0.f, 0.f, move_speeds, run_params);
}
